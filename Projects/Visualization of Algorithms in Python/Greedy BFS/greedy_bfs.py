...
def getHeuristics(G):
	heuristics = {}
	f = open('D://Visualization-of-Algorithms-in-Python/Greedy BFS/heuristics.txt')
	for i in G.nodes():
		node_heuristic_val = list(f.readline().split())
...
def CreateGraph():
	G = nx.Graph()
	f = open('D://Visualization-of-Algorithms-in-Python/Greedy BFS/input.txt')
	n = int(f.readline())
	for i in range(n):
		graph_edge_list = list(f.readline().split())
